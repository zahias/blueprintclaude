...
export default function CoordinatorCoursesPage() {
  ...
  function startEdit(course: Course) {
    setForm({ majorId: course.majorId, code: course.code, name: course.name, description: course.description || "" });
  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {!editingId && (
              <div>
                ...
                <select value={form.majorId} onChange={(e) => setForm({ ...form, majorId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" required>
            ...
            <div>
              ...
              <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" placeholder="e.g., CS201" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" placeholder="e.g., Data Structures" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
              <input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" placeholder="Optional" />
            </div>
          </div>
          <div className="flex gap-2">
            <button type="submit" className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition text-sm">{editingId ? "Update" : "Create"}</button>
            <button type="button" onClick={() => { setShowForm(false); setEditingId(null); }} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition text-sm">Cancel</button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">{majors.length === 0 ? "No majors assigned to you yet. Ask an admin to assign majors." : "No courses found."}</div>
      ) : (
              ...
              {filtered.map((course) => (
                <tr key={course.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <Link href={`/coordinator/courses/${course.id}`} className="text-teal-600 hover:text-teal-800 text-sm">Manage</Link>
                    <button onClick={() => startEdit(course)} className="text-teal-600 hover:text-teal-800 text-sm">Edit</button>
