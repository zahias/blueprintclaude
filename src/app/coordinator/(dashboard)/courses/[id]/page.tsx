...
export default function CoordinatorCourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function deleteLo(loId: string) {
    ...
    await fetch(`/api/courses/${id}/los/${loId}`, { method: "DELETE" }); loadCourse();
  }
  ...
  async function deleteTopic(topicId: string) {
    ...
    await fetch(`/api/courses/${id}/topics/${topicId}`, { method: "DELETE" }); loadCourse();
  }
    ...
    <div>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { setLoForm({ code: "", description: "" }); setEditingLoId(null); setShowLoForm(true); }} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700 transition">+ Add LO</button>
        </div>
          ...
          <form onSubmit={handleLoSubmit} className="bg-white rounded-xl border border-gray-200 p-4 mb-4 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div>
                ...
                <input value={loForm.code} onChange={(e) => setLoForm({ ...loForm, code: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" placeholder="LO1" required />
              </div>
              <div className="md:col-span-3">
                <label className="block text-xs font-medium text-gray-700 mb-1">Description</label>
                <input value={loForm.description} onChange={(e) => setLoForm({ ...loForm, description: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none" placeholder="Describe the learning outcome" required />
              </div>
            </div>
            <div className="flex gap-2">
              <button type="submit" className="px-3 py-1.5 bg-teal-600 text-white rounded-lg text-sm">{editingLoId ? "Update" : "Create"}</button>
              <button type="button" onClick={() => { setShowLoForm(false); setEditingLoId(null); }} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm">Cancel</button>
            </div>
          </form>
        )}
            ...
            {course.los.map((lo) => (
              <div key={lo.id} className="px-4 py-3 flex items-start justify-between">
                ...
                <div className="flex gap-2 ml-4 shrink-0">
                  <button onClick={() => { setLoForm({ code: lo.code, description: lo.description }); setEditingLoId(lo.id); setShowLoForm(true); }} className="text-teal-600 hover:text-teal-800 text-xs">Edit</button>
                  <button onClick={() => deleteLo(lo.id)} className="text-red-600 hover:text-red-800 text-xs">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {showTopicForm && (
          <form onSubmit={handleTopicSubmit} className="bg-white rounded-xl border border-gray-200 p-4 mb-4 space-y-3">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {course.los.map((lo) => (
                  <label key={lo.id} className="flex items-center gap-1.5 text-sm">
                    <input type="checkbox" checked={topicForm.loIds.includes(lo.id)} onChange={(e) => { const ids = e.target.checked ? [...topicForm.loIds, lo.id] : topicForm.loIds.filter((x) => x !== lo.id); setTopicForm({ ...topicForm, loIds: ids }); }} className="rounded border-gray-300" />
              ...
              {course.los.length === 0 && <p className="text-gray-400 text-xs mt-1">Add learning outcomes first to link them to topics.</p>}
            </div>
            <div className="flex gap-2">
              <button type="submit" className="px-3 py-1.5 bg-teal-600 text-white rounded-lg text-sm">{editingTopicId ? "Update" : "Create"}</button>
              <button type="button" onClick={() => { setShowTopicForm(false); setEditingTopicId(null); }} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm">Cancel</button>
            </div>
          </form>
        )}
