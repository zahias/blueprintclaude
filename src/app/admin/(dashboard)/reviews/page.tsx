...
export default function ReviewsPage() {
  ...
  async function loadBlueprints() {
    ...
    const url = `/api/blueprints?${params.toString()}`;
              ...
              {blueprints.map((bp) => (
                <tr key={bp.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right space-x-2">
                    <Link href={`/admin/reviews/${bp.id}`} className="text-indigo-600 hover:text-indigo-800 text-xs font-medium">
                      Review
                    </Link>
                    <button
                    ...
                    >
                      {copiedId === bp.id ? "Copied!" : "Copy Token"}
                    </button>
                  </td>
                </tr>
              ))}
